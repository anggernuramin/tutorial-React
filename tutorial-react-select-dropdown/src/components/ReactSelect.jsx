...
const ReactSelect = () => {
  ...
  //   muat pilihan dropdown dari hasil get api yg disimpan pada state datas
  //   DAN LAKUKAN SORT SEBELUM MELAKUKAN MAPPING,KARENA SORT MERETURN ARRAY YANG UTUH(AGAR METHOD localeCompare() BISA BERJALAN)
  // Data yg dibutuhkan react select adalah object yg didalamnya adalah value dan label
  const results = datas
  ...
  return (
    <>
      <button type="button" onClick={handleButton} style={{ marginBottom: "10px" }}>
      ...
      {/* cara memanggil Select,harus ada atribut options,dan jika ingin menambahkan className Dan data select akan ditampilkan kedalam property option pada komponen Select */}
      <Select className="item-menu" options={results} onChange={(e) => setUserSelect(e.value)} />
    </>
  );
};
